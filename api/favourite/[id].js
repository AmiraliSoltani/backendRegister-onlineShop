...
// Favourites Handler for dynamic id
const handler = async (req, res) => {
  ...
  const favouriteId = req.query.id; // Extract dynamic ID from the URL

  console.log("userId:", userId);
  console.log("favouriteId:", favouriteId);

  // Handle PUT requests to add a favourite product
  if (req.method === 'PUT') {
    try {
      const body = await parseBody(req); // Parse request body for additional data if needed

      const updatedFavourites = await userService.addFavourite(userId, favouriteId);
      res.status(200).json(updatedFavourites);
    } catch (err) {
      res.status(400).json({ error: err.message });
    }

    // Handle DELETE requests to remove a favourite product
  } else if (req.method === 'DELETE') {
    try {
      const updatedFavourites = await userService.removeFavourite(userId, favouriteId);
      res.status(200).json(updatedFavourites);
    } catch (err) {
